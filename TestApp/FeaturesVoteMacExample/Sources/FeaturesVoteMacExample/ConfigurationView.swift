...
/// macOS port of the iOS TestApp's `ConfigurationView`. Demonstrates the SDK's
/// `Configuration` (all `UI` + `Behavior` options), a few representative `Theme` colors,
/// user set/clear, and a button to present the SwiftUI `CreateFeatureView` as a sheet.
///
/// The SDK snapshots `theme`/`config` when a widget is first built, so after changing a
/// value here we call `demo.reloadWidgets()` to rebuild the widget tabs (see `DemoSettings`
/// in FeedbackWindow.swift). Change a setting, then switch to the Board or Roadmap tab to
/// see it take effect.
///
/// macOS differences vs. the iOS source: no `NavigationStack`/`.navigationTitle` (the
/// segmented bar is the navigation), `.formStyle(.grouped)` for the inset look, and the
/// Create sheet is given an explicit size and pinned to light (sheets have no intrinsic
/// size on macOS and `preferredColorScheme` does not reliably cross the sheet boundary).
